...
module.exports = function(options) {
  ...
  return function environment() {
    ...
    if (module) {
      file = path.join(dirname, module, 'all');
      script = scripts.resolve(path.resolve(file), extensions);

       if (existsSync(script)) {
         debug('configuring environment: %s', module+' - '+'all');
         require(script).call(this);
       }
    }

    file = path.join(dirname, module, env);
    script = scripts.resolve(path.resolve(file), extensions);

    if (existsSync(script)) {
      debug('configuring environment: %s', module+' - '+env);
